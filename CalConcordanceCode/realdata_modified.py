...
def analyze_real_data (outputfolder = 'fig201710/realdata/'):
    ...
    tau1 = np.repeat(0.025, N)
    tau2 = np.repeat(0.05, N)
    _, medium2XMM1Res = MCMCFit(medium2XMM, b, tau1, model = 'log_normal', df = 1.5, beta = 0.288**2)
...
def calculate_prior_infor(outputfolder = 'fig201710/realdata/'):
    ...
    weight = pd.DataFrame(np.vstack((OW1, OW2, NeW1, NeW2)).T,columns = ['O, tau=0.025', 'O, tau=0.05', 'Ne, tau=0.025', 'Ne, tau=0.05'],index = ['RGS1', 'MOS1', 'MOS2', 'pn', 'ACIS-S3', 'ACIS-I3',
    ...
    weight = pd.DataFrame(weight,columns = ['pn, tau=0.025', 'mos1, tau=0.025', 'mos2, tau=0.025','pn, tau=0.05', 'mos1, tau=0.05', 'mos2, tau=0.05'],index = ['hard band 2XMM', 'medium band 2XMM', 'soft band 2XMM',
    ...
    weight.to_csv(outputfolder+'priorinfluenceHMS.csv')
...
def makefig_real_data (outputfolder = 'fig201710/realdata/'):
    ...
    for i in np.arange(2):
        ...
        plt.errorbar(index - 0.125, data[i, :, 0, 0], yerr = data[i, :, 0, 1:3].T,fmt = None, ecolor = '#000000')
    ...
    for i in np.arange(4):
        ...
        y = np.array([data[0,ind[i]], data[1,ind[i]], data[2,ind[i]], d1[3+ind[i],0], d2[3+ind[i],0]])
        yerr = np.zeros((2, 5))
    ...
    for i in np.arange(3):
        ...
        for item in ([ax.title, ax.xaxis.label] + ax.get_xticklabels() + ax.get_yticklabels()):item.set_fontsize(15)
    #plt.show()
    plt.tight_layout()
...
def makefig_real_data_residue (outputfolder = 'fig201710/realdata/'):
    ...
    for i in np.arange(3):
        ...
        if i == 0:
            d = hardtRes.values
            #sigmaFit = 0.028**2*2/d[(N+M):(N*M+N+M),0].reshape((N, M))
            sigmaFit = (np.sqrt(0.028**2*2/hardtChain.values[:,(N+M):(N+M+N*M)]).mean(axis = 0).reshape((N, M)))**2
        
        if i == 1:
            d = mediumtRes.values
            #sigmaFit = 0.093**2*2/d[(N+M):(N*M+N+M),0].reshape((N, M))
            sigmaFit = (np.sqrt(0.093**2*2/mediumtChain.values[:,(N+M):(N+M+N*M)]).mean(axis = 0).reshape((N, M)))**2
        
        if i == 2:
            d = softtRes.values
            #sigmaFit = 0.026**2*2/d[(N+M):(N*M+N+M),0].reshape((N, M))
            sigmaFit = (np.sqrt(0.026**2*2/softtChain.values[:,(N+M):(N+M+N*M)]).mean(axis = 0).reshape((N, M)))**2
